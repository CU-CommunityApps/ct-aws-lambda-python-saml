...
TARGET_NETIDS = json.loads(os.environ.get('TARGET_NETIDS', EXAMPLE_TARGET_NETIDS))

SAML_ATTRIBUTE_NETID = 'urn:oid:0.9.2342.19200300.100.1.1'

def get_saml_request_info(event):
    return {
        "http_host": SAML_SP_HOSTNAME,
        "script_name": "/saml/consume",
        "server_port": "443",
        "get_data": "",
        "post_data": event['body'],

        # Advanced request options
        "https": "on", # defaults to "off"
        # "lowercase_urlencoding": "", # defaults to false
        # "request_uri": "",
        # "query_string": ""
    }
...
<body>
  ...
  <p><a href='/saml/test'>Manually POST a SAML response for testing</a></p>
  <p>
...
<body>
  ...
  <form action="/saml/consume" method="post">
    <p>
        RelayState:<br />
        <input name="RelayState" type="text" size="80"/>
    </p>
    <p>
        SAMLResponse:<br />
        <textarea name="SAMLResponse" rows="20" cols="80"></textarea>
    </p>
    <p>
...
def saml_consume(event, cookies):
    ...
    try: 
        ...
        processed_saml_response.is_valid(get_saml_request_info(event), raise_exceptions=True)
...
def lambda_handler(event, context):
    ...
    if path == SAML_SP_INTERNAL_PATH_PREFIX  + '/index.html':
        ...
        response = respond_html(get_saml_test_form_html(event), cookies)
    else:
